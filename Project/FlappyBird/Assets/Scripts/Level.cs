...
public class Level : MonoBehaviour
{
    ...
    private const float PIXEL_ERROR = 0.11f;
    ...
    private void Awake()
    {
        instance = this;
        SpawnInitialGround();
        transform.Find("SkyLimiter").position = new Vector3(0f, CAMERA_ORTHO_SIZE * 1.07f, 0f);
    ...
    private void CreatePipe(float height, float xPosition, bool createBottom)
    {
        ...
        if (createBottom)
        {
            pipeHeadYPosition = -CAMERA_ORTHO_SIZE + height + PIXEL_ERROR - PIPE_HEAD_HEIGHT * .5f;
        }
        else
        {
            pipeHeadYPosition = +CAMERA_ORTHO_SIZE - height - PIXEL_ERROR + PIPE_HEAD_HEIGHT * .5f;
